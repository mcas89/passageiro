...
function bloquearNaoNumeros(input) {
...
function autocompleteEndereco(input, sugestaoId) {
  ...
  debounceTimeout = setTimeout(() => {
    ...
    // Esconde sugestões se o valor for curto
    if (valor.length < 4) {
      sugestoes.innerHTML = '';
    ...
    // Escapa o valor para regex seguro
    const escapedValor = valor.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const regex = new RegExp(`(${escapedValor})`, 'i');
      ...
      .then(data => {
        ...
        data.features.forEach(feature => {
          ...
          div.onclick = () => {
            ...
            sugestoes.innerHTML = '';
            sugestoes.classList.add('hidden');
            ultimoValor = textoCompleto; // Atualiza o último valor
          };
...
function calcularPreco(distanciaKm) {
  ...
  const precoPorKm = 1.30;
  const precoTotal = precoBase + (precoPorKm * distanciaKm);
  return precoTotal;
}
...
function solicitarCorrida() {
    ...
    const metodoPagamento = document.getElementById("metodoPagamentoTexto").textContent;
    if (!metodoPagamento || metodoPagamento === "Método de pagamento") {
        mostrarPopup('Selecione um método de pagamento antes de solicitar a corrida.', 3000);
        return;
    }
    ...
    fetch(`${firebaseURL}/${cpfPassageiro}.json`)
        ...
        .then(data => {
            ...
            const novaCorrida = {
                ...
                motorista: "",
                bairroPassageiro,
                metodoPagamento // ← Adicionando ao objeto da corrida
            };
...
function verificarAceiteMotorista(cpfPassageiro) {
  ...
  if (window.temporizadorInterval) return;
      ...
      .then(data => {
        ...
        if (data.status === 'aceita' && data.motorista) {
          if (!window.motoristaExibido || window.motoristaAtual !== data.motorista) {
            ...
            // Mostrar botão e info do motorista
            document.querySelector('button[onclick="finalizarCorrida()"]').style.display = 'block';
            document.getElementById('infoMotorista').classList.remove('hidden');

            // Inicia acompanhamento contínuo da posição
            verificarLocalizacaoMotorista(cpfPassageiro);

            // Exibe mapa com partida e destino (a posição do motorista será atualizada depois)
            mostrarMapa(data.partida, data.destino);
          }
          ...
          if (window.motoristaExibido) {
            ...
            document.getElementById('loadingMotorista').classList.remove('hidden');
            document.getElementById('infoMotorista').classList.add('hidden');
            document.querySelector('button[onclick="finalizarCorrida()"]').style.display = 'none';
            mostrarPopup('Buscando novo motorista...', 3000);

            if (window.intervaloMotorista) {
              clearInterval(window.intervaloMotorista);
              window.intervaloMotorista = null;
            }
          }
...
function abrirMetodoPagamento() {
  document.getElementById("cardMetodoPagamento").classList.remove("hidden");
}

function fecharCardPagamento() {
  document.getElementById("cardMetodoPagamento").classList.add("hidden");
}

function selecionarMetodo(metodo) {
  const texto = document.getElementById("metodoPagamentoTexto");
  const iconeTopo = document.getElementById("iconeMetodoPagamento");
  const iconeResumo = document.getElementById("iconeResumoPagamento");

  texto.textContent = metodo;

  let classeIcone;

  switch (metodo) {
    case "Débito":
      classeIcone = "fas fa-credit-card";
      break;
    case "Crédito":
      classeIcone = "far fa-credit-card";
      break;
    case "Pix":
      classeIcone = "fas fa-qrcode";
      break;
    case "Dinheiro":
      classeIcone = "fas fa-money-bill-wave";
      break;
    default:
      classeIcone = "fas fa-coins";
  }

  iconeTopo.className = classeIcone;
  iconeResumo.className = classeIcone;

  fecharCardPagamento();
}

function verificarLocalizacaoMotorista(cpfPassageiro) {
  const url = `https://cardosoborracharia-a8854-default-rtdb.firebaseio.com/corridas/${cpfPassageiro}.json`;

  if (window.intervaloMotorista) clearInterval(window.intervaloMotorista);

  window.intervaloMotorista = setInterval(() => {
    fetch(url)
      .then(res => res.json())
      .then(data => {
        if (!data || !data.motorista_lat || !data.motorista_lng) return;

        const latitude = data.motorista_lat;
        const longitude = data.motorista_lng;

        const motoristaIcon = L.divIcon({
          html: '<i class="fas fa-car fa-2x" style="color: green;"></i>',
          iconSize: [30, 30],
          iconAnchor: [15, 30],
          className: 'custom-icon'
        });

        if (!window.mapaCorrida) return;

        if (window.marcadorMotorista) {
          window.marcadorMotorista.setLatLng([latitude, longitude]);
        } else {
          window.marcadorMotorista = L.marker([latitude, longitude], { icon: motoristaIcon })
            .addTo(mapaCorrida)
            .bindPopup("Motorista");
        }
      })
      .catch(err => console.error("Erro ao buscar localização do motorista:", err));
  }, 4000);
}
